  ...
  protected function _runPre()
  {
    ...
    if( null === $this->_profileFieldsMap &&
      ...
      $stmt = $fromDb->select()
        ...
        ->joinLeft('se_' . $this->_fromResourceType . 'cats', $this->_fromResourceType . 'field_' . $this->_fromResourceType . 'cat_id=' . $this->_fromResourceType . 'cat_id', $this->_fromResourceType . 'cat_order')
        ->order($this->_fromResourceType . 'cat_order ASC')
    ...
    if( null === $this->_profileSearchColumns &&
      ...
      foreach( $stmt->fetchAll() as $tmpFieldMeta ) {
        ...
        try {
          ...
          $this->_error($e->getMessage() . ', SQL was: ' . $keySql);
          continue;
        }
  ...
  protected function _translateRow(array $data, $key = null)
  {
    ...
    foreach( $profileValues as $key => $value ) {
      ...
      if( isset($this->_profileFieldsOptionsMap[$valFieldId]) ) {
        // Nothing selected
        if( $value == -1 || '' === $value /* ||  Not sure about empty  empty($value) */ ) {
          continue;
        }
