...
class Activity_IndexController extends Core_Controller_Action_Standard
{
  ...
  public function viewlikeAction()
  {
    ...
    public function viewdisLikeAction()
  {
    // Collect params
    $action_id = $this->_getParam('action_id');
    $viewer = Engine_Api::_()->user()->getViewer();
    ...
    if( null === $this->_getParam('format', null) ) {
      ...
      $this->view->body = $this->view->activity($action, array('viewAllDisLikes' => true, 'noList' => $this->_getParam('nolist', false)));
    }
  }
    ...
    public function disLikeAction()
  {
    ...
    // Start transaction
    $db = Engine_Api::_()->getDbtable('disLikes', 'activity')->getAdapter();
    $db->beginTransaction();
    ...
    try {
      ...
      if( !$comment_id ) {
        ...
        // Check authorization
        if( $action && !Engine_Api::_()->authorization()->isAllowed($action->getObject(), null, 'comment') ) {
          throw new Engine_Exception('This user is not allowed to disLike this item');
        }
        ...
        if( $action->subject_type == 'user' && $action->subject_id != $viewer->getIdentity() ) {
          ...
          Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($actionOwner, $viewer, $action, 'disLiked', array(
            'label' => 'post'
          ));
        }
      ...
      // Comment
        ...
        $comment->disLikes()->addDisLike($viewer);

        // @todo make sure notifications work right
        if( $comment->poster_id != $viewer->getIdentity() ) {
          Engine_Api::_()->getDbtable('notifications', 'activity')
              ->addNotification($comment->getPoster(), $viewer, $comment, 'disLiked', array(
                'label' => 'comment'
              ));
        }
      ...
      // Stats
      Engine_Api::_()->getDbtable('statistics', 'core')->increment('core.disLikes');
    ...
    // Success
    $this->view->status = true;
    $this->view->message = Zend_Registry::get('Zend_Translate')->_('You now disLike this action.');
    ...
    public function undisLikeAction()
  {
    ...
    try {
      ...
      if( !$comment_id ) {
        ...
        // Check authorization
        if( !Engine_Api::_()->authorization()->isAllowed($action->getObject(), null, 'comment') ) {
          throw new Engine_Exception('This user is not allowed to undisLike this item');
        }
      ...
      // Comment
        ...
        $comment->disLikes()->removeDisLike($viewer);
      }
    ...
    // Success
    $this->view->status = true;
    $this->view->message = Zend_Registry::get('Zend_Translate')->_('You no longer disLike this action.');
  ...
  {     
    ...
    try
    {
      ...
      foreach( $action->likes()->getAllLikesUsers() as $notifyUser )
      {
	    ...
	    foreach( $action->likes()->getAllDisLikesUsers() as $notifyUser )
      {
        if( $notifyUser->getIdentity() != $viewer->getIdentity() && $notifyUser->getIdentity() != $actionOwner->getIdentity() )
        {
          $notifyApi->addNotification($notifyUser, $viewer, $action, 'disLiked_commented', array(
            'label' => 'post'
          ));
        }
      }
  ...
  public function getLikesAction()
  {
	...
	$disLikes = $comment->disLikes()->getAllDisLikesUsers();
    $this->view->body = $this->view->translate(array('%s disLikes this', '%s disLike this',
      count($likes)), strip_tags($this->view->fluentList($disLikes)));
    $this->view->status = true;
  }
}