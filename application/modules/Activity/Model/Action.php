...
class Activity_Model_Action extends Core_Model_Item_Abstract
{
  ...
  public function isViewerDisLike()
  {
    if( $this->comments()->getDisLikeCount() <= 0 )
    {
      return false;
    }

    return $this->comments()->isDisLike(Engine_Api::_()->user()->getViewer());
  }
  ...
  {
    if( null !== $this->_disLikes )
    {
      return $this->_disLikes;
    }

    return $this->_disLikes = $this->disLikes()->getAllDisLikes();
  }
  ...
  public function getCommentsLikes($comments, $viewer)
  {
    ...
    public function getCommentsDisLikes($comments, $viewer)
  {
    if( empty($comments) ) {
      return array();
    }
    if( !is_object($comments[0]) || 
        !method_exists($comments[0], 'DisLikes') ) {
      return array();
    }

    $DisLikes = $comments[0]->DisLikes();
    $table = $DisLikes->getReceiver();
    ...
    public function disLikes()
  {
    ...
    switch( $commentable ) {
      // Comments linked to action item
      default: case 0: case 1:
        return new Engine_ProxyObject($this, Engine_Api::_()->getDbtable('disLikes', 'activity'));
        break;

      // Comments linked to subject
      case 2:
        return $this->getSubject()->disLikes();
        break;

      // Comments linked to object
      case 3:
        return $this->getObject()->disLikes();
        break;
      ...
      case 4:
        ...
        return $attachments[0]->disLikes();
        break;
    }

    throw new Activity_Model_Exception('DisdisLikes handler undefined');
  }
  ...
  public function deleteItem()
  {
    ...
    if ($this->getTypeInfo()->commentable <= 1) {
	  ...
	  // delete all "disLikes"
      Engine_Api::_()->getDbtable('disLikes', 'activity')->delete(array(
        'resource_id = ?' => $this->action_id,
      ));
      $this->_disLikes = null;
