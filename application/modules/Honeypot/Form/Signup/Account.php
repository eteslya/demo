...
class Honeypot_Form_Signup_Account extends Engine_Form {
    ...
    public function init() {
        ...
        $honeypot = $settings->getSetting('honeypot.email');
        if (!$honeypot)
            $settings->reloadSettings();

        // Init form
        $this->setTitle('Create Account')
                ->setAttrib('class', $this->getAttrib('class') . ' registration_form');

        $this->addElement('Text', 'email', array(
            'label' => 'Email Address',
            'autocomplete' => 'off'
        ));
        $this->email->getDecorator('htmlTag2')->setOption('class', 'hidden_field');
        ...
        $this->$honeypot->getDecorator('Description')->setOptions(array('placement' => 'APPEND'));
        $this->$honeypot->getValidator('NotEmpty')->setMessage('Please enter a valid email address.', 'isEmpty');
        $this->$honeypot->getValidator('Db_NoRecordExists')->setMessage('Someone has already registered this email address, please use another one.', 'recordFound');

        // Add banned email validator
        $bannedEmailValidator = new Engine_Validate_Callback(array($this, 'checkBannedEmail'), $this->$honeypot);
        $bannedEmailValidator->setMessage("This email address is not available, please use another one.");
        $this->$honeypot->addValidator($bannedEmailValidator);

        if (!empty($inviteSession->invite_email)) {
            $this->$honeypot->setValue($inviteSession->invite_email);
        }
        ...
        if ($settings->getSetting('user.signup.inviteonly') > 0) {
            $codeValidator = new Engine_Validate_Callback(array($this, 'checkInviteCode'), $this->$honeypot);
        ...
        $defaultLanguage = ucfirst(Engine_Api::_()->getApi('settings', 'core')->getSetting('core.locale.locale', 'en')); //et
        ...
        foreach ($languageList as $localeCode) {
			...
			if (ucfirst(Zend_Locale::getTranslation($localeCode, 'language', $localeCode)) == "русский")
			{
				$languageNameList[$localeCode] = "Русский";
			}
			else
			{
				$languageNameList[$localeCode] = ucfirst(Zend_Locale::getTranslation($localeCode, 'language', $localeCode)); //et "x1".
			}
			
            if (empty($languageNameList[$localeCode])) {
                list($locale, $territory) = explode('_', $localeCode);
                $languageNameList[$localeCode] = "{$territoryDataList[$territory]} {$languageDataList[$locale]}"; //et
            }
        }
