...
class Marketplace_IndexController extends Core_Controller_Action_Standard {
    ...
    public function init() {
		if(Engine_Api::_()->marketplace()->authorizeIsActive()){
			Engine_Api::_()->getApi('settings', 'core')->setSetting('marketplace.currency', 'USD');
		}
    ...
    public function viewAction() {
		...
		//et
		//$this->headScript()->prependFile($this->layout()->staticBaseUrl . 'et.js');
		...
		if(Engine_Api::_()->marketplace()->authorizeIsActive()){
			...
			$myAuthorize->addField('x_Receipt_Link_URL', 'https://'.$_SERVER['HTTP_HOST'].$this->view->baseUrl().'/marketplaces/paymentreturn/payment/authorize');
			$myAuthorize->addField('x_Relay_URL', 'https://'.$_SERVER['HTTP_HOST'].$this->view->baseUrl().'/marketplaces/paymentnotify/payment/authorize'); //https://'.$_SERVER['HTTP_HOST'].$this->view->baseUrl().'/marketplaces/paymentnotify/payment/authorize
			$myAuthorize->addField('x_Description', $marketplace->getTitle());
			$myAuthorize->addField('x_Amount', $total_amount);
			$myAuthorize->addField('x_Invoice_num', $marketplace->getIdentity().'-1:'.$viewer->getIdentity());
			$myAuthorize->addField('x_product_id', $marketplace->getIdentity());
			$myAuthorize->addField('x_user_id', $viewer->getIdentity());
			//$myAuthorize->addField('x_subscription_paynum', number_format(1, 2)); //et
			//$myAuthorize->addField('x_recurring_billing', number_format(1, 2)); //et
			if(intval($testmode))
    ...
    public function paypal($params = array()) {
        ...
        $paypal->setNumber($this->_getParam('marketplace_id') . '-1:' . $viewer->getIdentity());
    ...
    public function paymentnotifyAction() {
		...
		if($this->_getParam('payment') == 'authorize'){
			...
			$order = explode(':', $myAuthorize_order['x_invoice_num']);
			if(strstr($order[0], '|')){
			...
			if ($myAuthorize->validateIpn())
			{
				...
				foreach($item_ids as $key => $item_count_id){
					...
					$item_id = (!empty($item_id_info[0])?$item_id_info[0]:0);
					$item_count = (!empty($item_id_info[1])?$item_id_info[1]:1);
					...
					////////////////////////////////////////////////////////////discount
					if(Engine_Api::_()->marketplace()->couponIsActive()){
						$coupon_discount = Engine_Api::_()->marketplace()->getDiscount($marketplace->getIdentity(), $myAuthorize_order['x_user_id']);
					}else{
						$coupon_discount = 0;
					}
			...
			print_r($arrPost);
			$c = 'FAILURE' . "\n" . ob_get_clean();
			file_put_contents(APPLICATION_PATH . '/temporary/log/paypal.log', $c);
			...
			if ($paypal->validateNotify($arrPost)) {
				...
				if(strstr($order[0], '-')){//few items
					...
					foreach($item_ids as $key => $item_count_id){
						...
						////////////////////////////////////////////////////////////discount
						if(Engine_Api::_()->marketplace()->couponIsActive()){
							$coupon_discount = Engine_Api::_()->marketplace()->getDiscount($marketplace->getIdentity(), $order[1]);
						}else{
							$coupon_discount = 0;
						}
						if(Engine_Api::_()->marketplace()->upsIsActive()){
    ...
    public function editAction() {
        ...
        try {
            ...
            $cover = (!empty($values['cover'])?$values['cover']:'');
    ...
    public function getNavigation($active = false) {
        if (is_null($this->_navigation)) {
            ...
            if ($this->_helper->api()->user()->getViewer()->getIdentity()) {
				...
				if(Engine_Api::_()->marketplace()->couponIsActive() && $this->_helper->requireAuth()->setAuthParams('marketplace', null, 'create')->checkRequire()){
					$navigation->addPage(array(
    ...
    public function checkoutAction() {
		...
		if($cartitems->toArray()){
			...
			if(Engine_Api::_()->marketplace()->authorizeIsActive()){
				...
				//$myAuthorize->addField('x_subscription_paynum', number_format(1, 2)); //et
				//$myAuthorize->addField('x_recurring_billing', number_format(1, 2)); //et
				//$myAuthorize->addField('x_Currency_code', Engine_Api::_()->getApi('settings', 'core')->getSetting('marketplace.currency', 'USD'));
				if(intval($testmode))
					$myAuthorize->enableTestMode();
				$this->view->paymentForm = $myAuthorize->render();
			}else{
