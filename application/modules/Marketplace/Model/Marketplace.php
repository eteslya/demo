...
class Marketplace_Model_Marketplace extends Core_Model_Item_Abstract
{
 ...
 public function getHref2($params = array()) //et
  {
    return $this->marketplace_id;
  }

  public function getHref3($params = array()) //et
  {
    return $this->marketplace_id;
  }
  public function getChannelTypeURL2($page_id) //et
  {
	try
	{
    ...
    $prefix = $table->getTablePrefix();
		...
		$sql = "SELECT distinct LOWER(title) value
			FROM {$prefix}marketplace_marketplaces AS m 
			where m.marketplace_id = {$page_id}";
	...
	if(strpos($rows[0]['value'], 'trainer') > 0)
	{
		$i=1;
	}
	else if(strpos($rows[0]['value'], 'professional') > 0)
	{
		$i=2;
	}
	else if(strpos($rows[0]['value'], 'master') > 0)
	{
		$i=3;
	}
    return $photo_links[$i]."?i=".$i;
