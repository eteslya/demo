...
function fOwner(arg)
{
	...
	//document.getElementById("global_content").innerHTML = '<iframe id="body" name="body" src="'+arg+'" frameborder="0" scrolling="no" align="top" height="1008" width="798"></iframe>';

	window.open( "https://bio-views.com/bv/profile/"+arg); //<?php echo Engine_Api::_()->user()->getViewer()->getIdentity(); ?>" target="_new">;
	//document.filter_form.target = "_new";
...
function fSub(arg, pageTitle)
{
	...
	document.getElementById("global_content").innerHTML = '<iframe id="body" name="body" src="'+arg+'" frameborder="0" scrolling="no" align="top" height="1408" width="798"></iframe>';

	document.filter_form.action = arg;
	document.filter_form.target = "global_content";
...
function fParent()
{
	document.forms[0].action = "/bv/browse-pages";
...
<?php //et
		...
		if(isset($_REQUEST['c'])) //et subscription
		{
			echo '<script>document.getElementById("page-abc-wrapper").className = "hid";</script>';
			echo '<script>document.getElementById("main_tabs").innerHTML = "";</script>';
			
			echo "My Subscriptions:";
		}
?>
  ...
  <div class="tabs_alt tabs_parent">
	...
	<li class="<?php echo 'active'; ?>">
        <a class="page_sort_buttons"
           id="page_sort_sub"
           href="<?php echo $this->url(array('sort_type'=>'sort', 'sort_value'=>'sub'), 'page_browse_sort'); ?>"
           onclick="page_manager.setSort('sub'); return false;"><?php echo $this->translate("My Subscriptions")?></a>
      </li>
	<?php  }
	  ?>
	  
      <li class="<?php if ($this->sort == 'recent' && !isset($_REQUEST['c'])) echo 'active'; ?>">
        <a class="page_sort_buttons"
           id="page_sort_recent"
...
<?php if( $this->count > 0 ): ?>
  ...
  <ul class="page_list_items" id="page_list_cont" style="display:<?php echo  ($this->view == 'list') ? 'block' : 'none'; ?>;">
    ...
    <?php foreach( $this->paginator as $page ): ?>
	  ...
	  //echo "ZZZ ".Engine_Api::_()->user()->getViewer()->getIdentity()."/".$page_id." ".$page->isSubscribed(Engine_Api::_()->user()->getViewer()->getIdentity(), $page_id)."; ";

	  
	  if( ( !isset($_REQUEST['c']) && $page->isSubscribed(Engine_Api::_()->user()->getViewer()->getIdentity(), $page_id) == 0) || ( isset($_REQUEST['c']) && $page->isSubscribed(Engine_Api::_()->user()->getViewer()->getIdentity(), $page_id) != '0') )
	 {
		?>
      ...
      <li class="<?php if($page->sponsored) echo 'page_list_item_sponsored' ?> <?php echo $classBackgroundColor; ?>" >
        ...
        <div class="page_list_item_photo <?php  if($page->featured) echo 'featured_page' ?>">
          <a href="<?php echo $page->getHref()?>">
            <!-- original (//et) span style="background-image: url(--><?php //echo $page->getPhotoUrl('thumb.normal'); ?><!--);"-->
			<span style="background-image: url(<?php echo $page->getPhotoUrl2($page->getChannelType($page_id)); ?>);">
            </span>
		 ...
		 <?php
		 $top = " position: relative; top: -42px; ";
		 ?>
		 
          <?php endif;?>
		  
		  
		  <?php if(isset ($page->channel_level)){echo "zzz".$page->channel_level;} 
		  /*
			$subject = Engine_Api::_()->core()->getSubject(); //Get user

			$fields = $subject->fields()->getFieldsValuesByAlias();
			$channel_level = $fields['channel_level'];

			//$this->view->channel_level = $channel_level;
			echo "qqq".$channel_level;
			*/
			try
			{
			  //$field = Engine_Api::_()->getItem('field', $this->_getParam('field_id'));
			 //echo "nnn".$page->getChannelType();
			 
			 //echo "N:".$page->getChannelType($page_id)."/.".$page_id;
			 
			  //echo "mmm".$this->view->fieldsByAlias = Engine_Api::_()->fields()->getFieldsValuesByAlias("field");
			}
			catch(Exception $e)
			{
				echo $e;
		  ...
		  { 
					...
					if($page->getChannelTypeURL($page_id) != '0')
					{
						...
						<div style="<?php echo $top; ?> margin: 1px 0 1px 20px; padding:2px; color:#fff; text-align:center; width: 105px; background-color:#2D363F; cursor:pointer" onclick="fOwner('<?php echo $page->getChannelOwner($page_id) ?>');" style="color:#fff">Owner Info</div>
							...
							{?>
								...
								<a href="/bv/login" target="_parent" style="<?php echo $top; ?>border: none; margin:0; padding:0"><div style="margin: 1px 0 1px 20px; padding:2px; color:#fff; text-align:center; width: 105px; background-color:#2D363F; cursor:pointer" target="global_content" style="color:#fff">Sign In</div></a>
								
								
								<?php
							
							}
							else if ($page->getChannelType($page_id) != '5')
								{
								?>
								<div style="<?php echo $top; ?> margin: 1px 0 28px 20px; padding:2px; color:#fff; text-align:center; width: 105px; background-color:#2D363F; cursor:pointer" onclick="fSub('<?php echo $page->getChannelTypeURL($page_id); ?>', '<?php echo $page->getTitle(); ?>');" target="global_content" style="color:#fff">Subscribe</div>
								<?php
								
									$title_subscribe = ' onclick="fSub(\''.$page->getChannelTypeURL($page_id).'\', \''.$page->getTitle().'\'); ';
								}
								?>
		  ...
		  {
			?>
		  <div style="<?php echo $top; ?>margin: 2px 0 0 20px; padding:5px; color:#fff; text-align:center; width: 105px; background-color:#2D363F; cursor:pointer" onclick="fSub('<?php echo $page->getChannelTypeUnsub($page_id); ?>', '<?php echo $page->getTitle(); ?>');" target="global_content" style="color:#fff">UnSubscribe</div>
		  <?php
		  }
		  ?>
		  
		  <?php if($page->featured):?>
          <!--div class="page_featured" style="z-index:51">
            <span style="z-index:51"><?php echo $this->translate('Featured')?></span>
          </div-->
          <?php endif;?>
        </div>
        ...
        <div class="page_list_item_info">
          ...
          <div class="page_list_title">
            <a href="<?php  
					...
					{
						echo 'javascript:void(0);" '.$title_subscribe;
					}
  ...
  <?php
	  //et
		...
		if (isset($_REQUEST['h_trans_id']) && $_REQUEST['h_trans_id'] != '' )
		{
			///echo '<script>alert("'.$_REQUEST['h_trans_id'].'")</script>';
			
			$res = $page->setSubscription($_REQUEST['h_page_id'], $_REQUEST['h_trans_id'], $_REQUEST['h_fname'], $_REQUEST['h_lname'], $_REQUEST['h_card'],$_REQUEST['h_exp']);
			
		}
		else if (isset($_REQUEST['h_cancel_id']) && $_REQUEST['h_cancel_id'] != '' )
		{
			$res = $page->cancelSubscription($_REQUEST['h_cancel_id']);
		}
	  ?>
