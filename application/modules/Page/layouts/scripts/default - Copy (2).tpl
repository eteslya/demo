...
<head>
  ...
  function fSub(arg)
{
	document.getElementById("global_content").innerHTML = '<iframe id="body" name="body" src="'+arg+'" frameborder="0" scrolling="no" align="top" height="1308" width="798"></iframe>';
...
<body id="global_page_<?php echo $request->getModuleName() . '-' . $request->getControllerName() . '-' . $request->getActionName() ?>">
  ...
  <div id='global_wrapper'>
    <div id='global_content'>
	  ...
	  if($this->page->isPaid(Engine_Api::_()->user()->getViewer()->getIdentity(), $this->page->getIdentity()) == 0)
		{
			
			//echo $this->page->getIdentity()."!!!".$this->page->getChannelTypeURL($this->page->getIdentity());
			
			//echo "<script>alert(location.href);</script>";
			
			echo '<script>fSub("'.$this->page->getChannelTypeURL($this->page->getIdentity()).'");</script>';
  ...
  <div id="global_footer">
    <?php
		...
		//echo "ZZZ 5 ".Engine_Api::_()->user()->getViewer()->getIdentity(). " : " . $this->page->getIdentity()." :: ". $this->page->getChannelType($this->page->getIdentity());
		//echo "Q:".$this->page->isPaid(Engine_Api::_()->user()->getViewer()->getIdentity(), $this->page->getIdentity());
		/*
		if($this->page->isPaid(Engine_Api::_()->user()->getViewer()->getIdentity(), $this->page->getIdentity()) == 0)
		{
			
			echo "!!!"; //.document.forms[0].action."?"; //$this->page->getChannelTypeURL($this->page->getIdentity());
