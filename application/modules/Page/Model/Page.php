...
class Page_Model_Page extends Core_Model_Item_Abstract
{
  ...
  public function getPhotoUrl2($type)
  {
	
	$photo_links = array('', '', '/bv/public/hebadge_badge/e4/00e4_97e4.png?c=8f30', '/bv/public/hebadge_badge/dc/00dc_2190.png?c=969c',
	'/bv/public/hebadge_badge/d8/00d8_23b8.png?c=e976', '/bv/public/hebadge_pagebadge/05/01/0104_f6ba.png',
	'/bv/public/hebadge_badge/e4/00e4_97e4.png?c=8f30', '/bv/public/hebadge_badge/dc/00dc_2190.png?c=969c',
	'/bv/public/hebadge_badge/d8/00d8_23b8.png?c=e976'); //'/bv/public/hebadge_badge/50/07/0749_13b3.jpg', 
	
	if ( $type == '' || $type < 5 || $type > 8)
	{
		$type = 2;
	}
	
	return $photo_links[$type]."?t=".$type;
  }
  ...
  public function isPaid($userId, $pageId)
  {
	try
	{
	...
	$prefix = $table->getTablePrefix();
		...
		union 
		select 1 status from {$prefix}page_pages pp
			where pp.page_id = {$pageId} and pp.user_id = {$userId}
		union
		select 1 status from {$prefix}users u
			where u.user_id = {$userId} and u.level_id in (1, 2)
			

		union
		select 1 status from {$prefix}page_pages pp
			where page_id = {$pageId} and package_id = 5
   ...
   public function isSubscribed($userId, $pageId)
  {
	try
	{
	...
	$prefix = $table->getTablePrefix();
		...
		SELECT distinct ifnull( status, 0) status
		FROM business b
        where page_id={$pageId} and user_id = {$userId} and status = 1 order by status";
  ...
  public function getChannelOwner($page_id)
  {
	try
	{
    ...
    $prefix = $table->getTablePrefix();
		 ...
		 $sql = "
		 SELECT distinct pp.user_id page_id
		FROM {$prefix}page_pages  pp
  ...
  public function getChannelTypeURL($page_id)
  {
	try
	{
		...
		$marketplace_links = array('', '', '', '', '', 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=5'.$email, 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=6'.$email,
		'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=7'.$email,
		'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=8'.$email,'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=9'.$email);
	  ...
	  $sql = "
		...
		else
		{
		  ...
		  /*
			 $sql = "
			 SELECT distinct v.value, v.field_id
		  FROM {$prefix}page_fields_values AS v
			LEFT OUTER JOIN {$prefix}page_fields_meta AS m
			  ON(v.field_id = m.field_id)
				where m.label = 'Channel Level' and v.item_id = {$page_id} order by 2 desc limit 1"; //{$this->getIdentity()}";
	...
	if ($rows[0]['package_id'] == 0)
	{
		return 0;
	}
	else
	{
		return $marketplace_links[$rows[0]['package_id']]."&p=".$page_id."&uid=".Engine_Api::_()->user()->getViewer()->getIdentity()."&pending=".$pending;
	}
 ...
 public function getChannelType($page_id)
  {
	try
	{
		...
		'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=1'.$email, 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=2'.$email,'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=3'.$email,
		'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=4'.$email,
		'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=5'.$email, 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=6'.$email,'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=7'.$email,'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=8'.$email);
	...
	$sql = "
	  ...
	  where page_id = {$page_id}";
	  /*
        LEFT OUTER JOIN {$prefix}page_packages AS pp
          ON(p.package_id = m.field_id)
			where m.label = 'Channel Level' and v.item_id = {$page_id} order by 2 desc limit 1";
			*/
 ...
 public function getChannelTypeUnsub($page_id)
  {
	try
	{
	
		$marketplace_links = array('', 'https://bio-views.com/bv/custom/auth/ch2.php?ch_id=1', 'https://bio-views.com/bv/custom/auth/ch2.php?ch_id=2','https://bio-views.com/bv/custom/auth/ch2.php?ch_id=3');
    ...
    $chId =  $rows[0]['package_id'];
	//
	
	
		 $sql = "
		 SELECT transaction_id, payment_method, ref_id
      FROM business b 
			where user_id = {$user_id} and b.page_id = {$page_id} and status = 1"; //{$this->getIdentity()}";
    ...
    return "https://bio-views.com/bv/custom/auth/ch2.php?uId=".$user_id."&ch_id=".$chId."&p=".$this->getIdentity()."&sub_id=".$rows[0]['transaction_id']."&pay_method=".$rows[0]['payment_method']."&ref_id=".$rows[0]['ref_id'];
   ...
   public function setSubscription($pageId, $sub_id, $fname, $lname, $card, $exp)
  {
	try
	{
    ...
    $prefix = $table->getTablePrefix();
	 ...
	 $user_id = (int)Engine_Api::_()->user()->getViewer()->getIdentity();
		 ...
		 $sql = "insert into business values(".$user_id.", ".$sub_id.", null, ".$pageId.", now(), '".$fname."', '".$lname."', '".$card."', '".$exp."', 'auth', 1)";
    ...
    public function cancelSubscription($sub_id)
  {
	...
	$sql = 'delete from business where transaction_id='.$sub_id;
