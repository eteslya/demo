...
class Page_Model_DbTable_Packages extends Engine_Db_Table
{
  ...
  public function getPackages( $params = array() )
  {
    ...
    if( !empty($params['payed']) ) {
      /**
       *  @var $orderTbl Payment_Model_DbTable_Orders
       */

      $orderTbl = Engine_Api::_()->getDbTable('orders', 'payment');
      $orderSelect = $orderTbl->select()
        ->from(array('o' => $orderTbl->info('name')), array('source_id'))
        ->where("o.state = 'complete'")
        ->where("o.source_type = 'page_subscription'")
        ->where("o.user_id = ? ", $viewer->getIdentity());
      ...
      $select->where('p.price > 0');
      $select->where('s.subscription_id IN ?', $orderSelect);
