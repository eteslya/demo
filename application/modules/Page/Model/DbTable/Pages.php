...
class Page_Model_DbTable_Pages extends Engine_Db_Table implements Engine_Content_Storage_Interface
{
  ...
  public function loadMetaData(Engine_Content $contentAdapter, $name)
  {
    ...
    if( !empty($params['cat_id']) && is_numeric($params['cat_id']) ) {
      $optionsTbl = Engine_Api::_()->getDbTable('fieldsValues', 'page');
      $select->setIntegrityCheck(false)
        ->join(array('options' => $optionsTbl->info('name')),
          "page_id = `options`.item_id AND `options`.value = " . intval($params['cat_id']));
    }
    ...
    if (!empty($params['adv_location'])) {
      ...
      $url = $http_prefix.'maps.googleapis.com/maps/api/geocode/json?address='.$params['adv_location'].'&sensor=false';
    ...
    $select->order($prefix.'page_pages.sponsored DESC');
		...
		if (isset($params['sort'])){
			...
			$user_id = (int)Engine_Api::_()->user()->getViewer()->getIdentity(); //et
			...
			switch ($params['sort']){
				case 'sub' :
					...
					$select
						->where('business.transaction_id is not null and business.user_id = '.$user_id.' and business.status = 1');
					$select
						->joinLeft('business', 'business.page_id = '.$prefix.'page_pages.page_id and business.user_id = '.$user_id.'  and business.status = 1', array());
					break;
				case 'recent' :
					$select
						->order($prefix.'page_pages.creation_date DESC');
