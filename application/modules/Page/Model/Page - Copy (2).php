...
class Page_Model_Page extends Core_Model_Item_Abstract
{
  ...
  public function getPhotoUrl2($type)
  {
	...
	$photo_links = array('', '/bv/public/hebadge_badge/e4/00e4_97e4.png?c=8f30', '/bv/public/hebadge_badge/dc/00dc_2190.png?c=969c','/bv/public/hebadge_badge/d8/00d8_23b8.png?c=e976');
		
	return $photo_links[$type];
  ...
  public function getChannelTypeURL($page_id)
  {
	try
	{
		...
		$email = "&email=".Engine_Api::_()->core()->getSubject('user')->email;

  
		$marketplace_links = array('', 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=1'.$email, 'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=2'.$email,'https://bio-views.com/bv/custom/auth/ch1.php?ch_id=3'.$email);
      ...
      FROM {$prefix}page_fields_values AS v
        LEFT OUTER JOIN {$prefix}page_fields_meta AS m
          ON(v.field_id = m.field_id)
			where m.label = 'Channel Level' and v.item_id = {$page_id}"; //{$this->getIdentity()}";
    ...
    return $marketplace_links[$rows[0]['value']]."&p=".$page_id;
 ...
 public function getChannelType($page_id)
  {
	try
	{
		...
		$marketplace_links = array('', 'http://bio-views.com/bv/custom/auth/ch1.php?ch_id=1'.$email, 'http://bio-views.com/bv/custom/auth/ch1.php?ch_id=2'.$email,'http://bio-views.com/bv/custom/auth/ch1.php?ch_id=3'.$email);
 ...
 public function getChannelTypeUnsub($page_id)
  {
	try
	{
		...
		$marketplace_links = array('', 'http://bio-views.com/bv/custom/auth/ch2.php?ch_id=1', 'http://bio-views.com/bv/custom/auth/ch2.php?ch_id=2','http://bio-views.com/bv/custom/auth/ch2.php?ch_id=3');
	...
	$user_id = (int)Engine_Api::_()->user()->getViewer()->getIdentity();
      ...
      FROM business b 
			where user_id = {$user_id} and b.page_id = {$page_id}"; //{$this->getIdentity()}";
    ...
    return "http://bio-views.com/bv/custom/auth/ch2.php?p=".$this->getIdentity()."&sub_id=".$rows[0]['subscription_id'];
   ...
   public function setSubscription($pageId, $sub_id, $fname, $lname, $card, $exp)
  {
    ...
    $prefix = $table->getTablePrefix();
	 ...
	 $user_id = (int)Engine_Api::_()->user()->getViewer()->getIdentity();
		 ...
		 $sql = "insert into business values(".$user_id.", ".$sub_id.",".$pageId.", now(), '".$fname."', '".$lname."', '".$card."', '".$exp."')";
    ...
    public function cancelSubscription($sub_id)
  {
	...
	$sql = 'delete from business where subscription_id='.$sub_id;
