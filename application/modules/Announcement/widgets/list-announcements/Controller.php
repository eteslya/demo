 ...
 * @version    $Id: Controller.php 10055 2013-06-13 00:23:37Z matthew $
 * @author     John
 */
...
class Announcement_Widget_ListAnnouncementsController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    for($i = 0; $i < $announcement_count; $i++) {
      ...
      if( $user_member_level_id != NULL && in_array( $user_member_level_id, $member_level_array) != FALSE ) {
        array_push($announcement_keep_list, $announcement_query[$i]);
        continue;
      }      
      ...
      if( $profile_type_id != NULL && in_array( $profile_type_id, $profile_type_array) != FALSE ) {
        array_push($announcement_keep_list, $announcement_query[$i]);
        continue;
      } 
    }
