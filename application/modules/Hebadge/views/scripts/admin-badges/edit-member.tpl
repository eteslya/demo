...
<script type="text/javascript">
  ...
  function hebadgeUseBadge(element, badge_id)
  {
    ...
    if (element.hasClass('hebadge_active')){
      Hebadge.request(en4.core.baseUrl + 'admin/hebadge/badges/edit-member/badge/' + badge_id + '/enabled/0/format/json/id/<?php echo $this->subject->getIdentity()?>');
      element.removeClass('hebadge_active');
    } else {
      Hebadge.request(en4.core.baseUrl + 'admin/hebadge/badges/edit-member/badge/' + badge_id + '/enabled/1/format/json/id/<?php echo $this->subject->getIdentity()?>');
...
<div class="hebadge_member_edit">
  ...
  <h3><?php echo $this->translate('HEBADGE_MEMBER_EDIT', array($this->subject->getTitle()))?></h3>
