 ...
 * @version    $Id: Pagebadge.php 02.04.12 09:12 michael $
 * @author     Michael
 */
...
class Hebadge_Model_Pagebadge extends Core_Model_Item_Abstract
{
  ...
  public function delete()
  {
    ...
    foreach (Engine_Api::_()->getDbTable('pagemembers', 'hebadge')->fetchAll(array('pagebadge_id = ?' => $this->getIdentity())) as $item){
      $item->delete();
    }
  }
  ...
  public function getHref($params = array())
  {
    $params = array_merge(array(
      'route' => 'page_browse_sort',
      'reset' => true,
      'badges' => $this->getSlug(),
      'badge' => $this->getIdentity()
    ), $params);
    $route = $params['route'];
    $reset = $params['reset'];
    unset($params['route']);
  ...
  public function getMember(Core_Model_Item_Abstract $owner)
  {
    ...
    $select = $table->select()
        ->where('pagebadge_id = ?', $this->getIdentity())
  ...
  {
    $membersTable = Engine_Api::_()->getDbTable('pagemembers', 'hebadge');
    ...
    if (!$member){
      ...
      $member->setFromArray(array(
        'pagebadge_id' => $this->getIdentity(),
        'page_id' => $owner->getIdentity(),
        'approved' => 0,
