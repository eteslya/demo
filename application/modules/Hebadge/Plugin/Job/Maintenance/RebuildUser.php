...
class Hebadge_Plugin_Job_Maintenance_RebuildUser extends Core_Plugin_Job_Abstract
{
  protected function _execute()
  {
    ...
    while( !$break && $count <= $limit ) {
      ...
      // Main
        ...
        foreach (Engine_Api::_()->hebadge()->getRequireList() as $key => $require){
          $plugin = Engine_Api::_()->hebadge()->getRequireClass($key);
          if (empty($plugin)){
            continue ;
          }
          if ($plugin){
        ...
        Engine_Api::_()->getDbTable('creditbadges', 'hebadge')->checkOwnerRank($item);
