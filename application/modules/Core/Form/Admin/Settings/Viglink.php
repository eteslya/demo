...
class Core_Form_Admin_Settings_Viglink extends Engine_Form
{
  public function init()
  {
    ...
    $description = vsprintf($description, array(
      ...
      'http://www.socialengine.com/client/viglink?id=' . urlencode($subid),
      'http://support.socialengine.com/questions/206/Viglink-Setup',
    ));
	$settings = Engine_Api::_()->getApi('settings', 'core');
	if( $settings->getSetting('user.support.links', 0) == 1 ) {
	  $moreinfo = $this->getTranslator()->translate( 
        'More Info: <a href="http://support.socialengine.com/questions/206/Viglink-Setup" target="_blank"> KB Article</a>');
	} else {
	  $moreinfo = $this->getTranslator()->translate( 
        '');
	}
