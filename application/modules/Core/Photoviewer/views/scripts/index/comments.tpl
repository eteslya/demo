...
<div class="external-options" style="display: none;">
  ...
  <?php if (Engine_Api::_()->getDbTable('settings', 'core')->getSetting('photoviewer.downloadable' , 1)):?>
    <?php
	  if(true){
      echo '<a href="'.$this->photo->getPhotoUrl('thumb.default').'" download="download" content="nofollow"><i class="icon-download"></i>' .$this->translate('PHOTOVIEWER_Download this photo').'</a>';
    }else{
      ?>
