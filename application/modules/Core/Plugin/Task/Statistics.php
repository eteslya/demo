...
class Core_Plugin_Task_Statistics extends Core_Plugin_Task_Abstract
{
  public function execute()
  {
	...
	try {
      $site['disLikes'] = 0;
      $site['disLikes'] += (int) $db->select()
          ->from('engine4_activity_disLikes', new Zend_Db_Expr('COUNT(disLike_id)'))
          ->query()
          ->fetchColumn();
      $site['disLikes'] += (int) $db->select()
          ->from('engine4_core_disLikes', new Zend_Db_Expr('COUNT(disLike_id)'))
          ->query()
          ->fetchColumn();
    } catch( Exception $e ) {}
