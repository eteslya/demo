 ...
 * @author     Minh Nguyen
 */
class Groupbuy_AdminPaymentController extends Core_Controller_Action_Admin
{
   protected $_paginate_params = array();
   public function init()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('groupbuy_admin_main', array(), 'groupbuy_admin_main_paymentsettings');
  }
  public function indexAction()
  {
       // Make paginator
    $select = Engine_Api::_()->getDbtable('gateways', 'groupbuy')->select();
  ...
  {
    // Get gateway
    $gateway = Engine_Api::_()->getDbtable('gateways', 'groupbuy')
      ->find($this->_getParam('gateway_id'))
      ->current();

    // Make form
    if($gateway['gateway_name'] == 'Paypal')
        $this->view->form = $form = new Groupbuy_Form_Admin_Payment_Paypal();
    else
   ...
   if( !$this->getRequest()->isPost() ) {
    ...
    $params['params'] = $val;
    $params['api_app_id'] ="";
    Groupbuy_Api_Gateway::saveSettingGateway($gateway['gateway_name'],$params);  
    $admin = Groupbuy_Api_Cart::getFinanceAccount(null,1,$gateway['gateway_id']);  
    $admin['account_username'] = $val['admin_account'];
    $admin['gateway_id'] = $gateway['gateway_id'];
    $admin['payment_type'] = 1;
    $admin = Groupbuy_Api_Cart::saveFinanceAccount($admin);
