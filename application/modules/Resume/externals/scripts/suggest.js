...
/* $Id: composer_resume.js 8986 2011-06-15 00:14:28Z john $ */


RadcodesResumeSuggest = new Class({
  ...
  options : {
    title : 'Add Resume',
    lang : {},
    // Options for the resume preview request
    requestOptions : {},
    ...
    // Delay to detect resumes in input
    monitorDelay : 600,
    debug : true
  },
  ...
  doSuggest : function() {
    ...
    var val = this.elements.formInput.value;
    if( !val ) {
      this.elements.formInput.addClass('required');		
      return;
    }
    this.elements.formInput.removeClass('required');
    if( !val.match(/^[a-zA-Z]{1,5}:\/\//) )
    ...
    $$('.compose-preview-images, .compose-preview-info').each(function(el){
    	el.destroy();
    });
  ...
  doShowPreview : function() {
    ...
    if( this.params.loadedImages.length > 0 ) {
      ...
      this.elements.previewImages = new Element('div', {
        'id' : 'compose-resume-preview-images',
    ...
    this.elements.previewInfo = new Element('div', {
      'id' : 'compose-resume-preview-info',
    ...
    if( this.params.loadedImages.length > 0 ) {
      this.elements.previewOptions = new Element('div', {
        'id' : 'compose-resume-preview-options',
      ...
      if( this.params.loadedImages.length > 1 ) {
        this.elements.previewChoose = new Element('div', {
          'id' : 'compose-resume-preview-options-choose',
        ...
        this.elements.previewPrevious = new Element('a', {
          'id' : 'compose-resume-preview-options-previous',
        ...
        this.elements.previewCount = new Element('span', {
          'id' : 'compose-resume-preview-options-count',
          'class' : 'compose-preview-options-count'
        }).inject(this.elements.previewChoose);


        this.elements.previewPrevious = new Element('a', {
          'id' : 'compose-resume-preview-options-next',
      ...
      this.elements.previewNoImageLabel = new Element('label', {
        'for' : 'compose-resume-preview-options-none-input',
