...
class Zend_Db_Adapter_Mysqli extends Zend_Db_Adapter_Abstract
{
    ...
    protected function _connect()
    {
        ...
        } elseif (FALSE !== strpos($this->_config['host'], ':')) {
            $port   = substr($this->_config['host'], strpos($this->_config['host'], ':')+1);
            $this->_config['host'] = substr($this->_config['host'], 0, strpos($this->_config['host'], ':'));
            if (!is_numeric($port)) {
              $socket = $port;
              $port   = null;
            } else {
        ...
        if (!empty($this->_config['charset'])) {
            // Webligo MySQL client lib < 4.1.11 compat
            if( function_exists('mysqli_set_charset') ) {
              mysqli_set_charset($this->_connection, $this->_config['charset']);
            } else {
              $this->query($this->quoteInto('SET NAMES ?', $this->_config['charset']));
            }
        }
    }
