    ...
    extends Zend_View_Helper_Navigation_HelperAbstract
{
    ...
    public function htmlify(Zend_Navigation_Page $page)
    {
		...
		$cl = ' id="menu_'.$page.'"';
        return '<' . $element . $cl .$this->_htmlAttribs($attribs) . '>'
                                          ...
                                          $maxDepth)
    {
        ...
        $ulClass = $ulClass ? ' class="et_a ' . $ulClass . '"' : '';
        ...
        foreach ($active['page'] as $subPage) {
            ...
            $liClass = $subPage->isActive(true) ? ' class="et_b active"' : '';
                                   ...
                                   $onlyActive)
    {
        ...
        foreach ($iterator as $page) {
			...
			$sub_menu["Find Channels"] = "Find Channels";
			$sub_menu["Invite People"] = "Invite People";
			$sub_menu["Albums"] = "Albums";
			$sub_menu["Videos"] = "Videos";
			$sub_menu["Blogs"] = "Blogs";
			$sub_menu["My Resume"] = "My Resume";
			$sub_menu["Find Jobs"] = "Find Jobs";
			$sub_menu["Seek Employees"] = "Seek Employees";
			
			$sub_menu["Create New Channel"] = "Create New Channel";
			$sub_menu["Promote Bio-Views"] = "Promote Bio-Views";
			$sub_menu["I-Payout eWallet"] = "I-Payout eWallet";
			$sub_menu["Channels"] = "Channels";
			$sub_menu["Online Stores"] = "Online Stores";
			$sub_menu["GroupBuy"] = "GroupBuy";
			
				if ($this->getUseTranslator() && $t = $this->getTranslator()) {
					foreach ($sub_menu as $sm)
					{
						if (is_string($sm) && !empty($sm)) {
							$sub_menu[$sm] = $t->translate($sm);
						}
					}
				}
            ...
            if ($depth > $prevDepth) {
                // start new ul tag
                if ($ulClass && $depth ==  0) {
                    $ulClass = ' id="menu" class="menu lgray_ slide ' . $ulClass . '"';
                } else {
                    $ulClass = '';
                }
            ...
            // Drop-down menus custom code insertion / render li tag and page
			if ($page == 'Home') //top menu name - 1st tab //ET added and AD changed
			{
				//insert custom sub-menu items here
				$sub_menu = '<ul class="sub_menu" >	
                                </ul>';
			}
			else if ($page == 'Activities') //top menu name - 2nd tab //AD added
			{
				//insert custom sub-menu items here
				$sub_menu = '<ul class="sub_menu sub_menu1" >
                               <li><a href="/bv/invite">'.$sub_menu["Invite People"].'</a></li> 
                               <li><a href="/bv/albums/manage">'.$sub_menu["Albums"].'</a></li>
				<li><a href="/bv/videos/manage">'.$sub_menu["Videos"].'</a></li>
                                <li><a href="/bv/blogs/manage">'.$sub_menu["Blogs"].'</a></li>
                                </ul>';
			}                          
                        else if ($page == 'Learn') //top menu name - 3th tab //AD added
			{
				//insert custom sub-menu items here
				// translate label and title?
				//$sub_menu = "Find Channels";
				
				
		
				$sub_menu = '<ul class="sub_menu sub_menu2" >
                                <li><a href="/bv/browse-pages/sort_type/sort/sort_value/sub?c=0">My Subscriptions</a></li>
				<li><a href="/bv/browse-pages">'.$sub_menu["Find Channels"].'</a></li>
                                </ul>';
			}
                        else if ($page == 'Jobs') //top menu name - 4rd tab //AD added
			{
				//insert custom sub-menu items here
				$sub_menu = '<ul class="sub_menu sub_menu3" >
                                <li><a href="/bv/resumes/manage">'.$sub_menu["My Resume"].'</a></li>
                                <li><a href="/bv/jobs">'.$sub_menu["Find Jobs"].'</a></li>
                                <li><a href="/bv/resumes">'.$sub_menu["Seek Employees"].'</a></li>
                                </ul>';
			}      
                        else if ($page == 'Business') //top menu name - 5th tab //AD added
			{
				//insert custom sub-menu items here
				$sub_menu = '<ul class="sub_menu sub_menu4" >
                                <li><a href="/bv/page-package/choose/page_id/0">'.$sub_menu["Create New Channel"].'</a></li>
                                <li><a href="/bv/affiliate">'.$sub_menu["Promote Bio-Views"].'</a></li>
                                <li><a href="https://bio-views.globalewallet.com/MemberLogin.aspx" target=_new>'.$sub_menu["I-Payout eWallet"].'</a></li>
                                </ul>';
			}      
                        else if ($page == 'Store') //top menu name - 6th tab //AD changed
			{
				//insert custom sub-menu items here  
				$sub_menu = '<ul class="sub_menu sub_menu5" >
                                <li><a href="/bv/browse-pages">'.$sub_menu["Channels"].'</a></li>
                                <li><a href="/bv/store/stores">'.$sub_menu["Online Stores"].'</a></li>
				<li><a href="/bv/group-buy">'.$sub_menu["GroupBuy"].'</a></li>
                                </ul>';
			}
                        else if ($page == 'MarketPlace') //top menu name - 7th tab //AD changed
			{
				//insert custom sub-menu items here
				$sub_menu = '<ul class="sub_menu sub_menu6" >
                                <li><a href="/bv/browse-pages">'.$sub_menu["Channels"].'</a></li>
                                <li><a href="/bv/store">'.$sub_menu["Online Stores"].'</a></li>
                                <li><a href="/bv/group-buy">'.$sub_menu["GroupBuy"].'</a></li>
                                </ul>';
			}  
                        else
			...
			//et
			$str = $this->htmlify($page);
			if(strpos($str, "globalewallet.com") > 0)
			{
				...
				$str = '<a href="javascript:void(0)" onclick="document.forms[1].innerHTML =\' '.$s.' \';" >eWallet/I-Payout</a>';
			}

			
            $liClass = $isActive ? ' class="et_d active"' : ' class="rel" ';
            $html .= $myIndent . '    <li' . $liClass . '>' . self::EOL
                   . $myIndent . '        ' . $str . $sub_menu . self::EOL;
